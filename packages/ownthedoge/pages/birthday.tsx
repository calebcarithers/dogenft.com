...
const SoulBound: React.FC = () => {
    ...
    const init = async () => {
      if (SoulBoundAbi && signer && isConnectedToCorrectNetwork) {
        const contract = new ethers.Contract(
          vars.NEXT_PUBLIC_SOULBOUND_CONTRACT_ADDRESS,
          SoulBoundAbi,
          signer
        );
  ...
  useEffect(() => {
    const getClaimedId = async () => {
      if (soulBoundContract && signer && isClaimed) {
        ...
        if (logs.length > 1) {
          throw new Error(
            "There should not be more than one transfer event here"
          );
        }
        ...
        if (logs.length == 1 && logs[0].args) {
          ...
          const claimedId = METDATAS.filter((item) => item.name === name)[0]
            ?.id;
          if (claimedId !== null) {
  ...
  const claim = async () => {
    ...
    if (soulBoundContract) {
      try {
        ...
        const proof = getProof(address, getSoulboundWhitelist());
  ...
  const getStatusText = useCallback(() => {
    if (signer) {
        ...
        return (
          <div className={css("text-2xl", "font-bold")}>
            <div className={css("mb-2")}>
              You claimed{" "}
              {METDATAS.filter((item) => item.id === claimedId)[0]?.name}!
            </div>
            <div className={css("my-6")}>
              ...
              <Link
                ...
                href={
                  isProduction()
                    ? "https://opensea.io/collection/dogs-first-birthday"
                    : "https://testnets.opensea.io/collection/dogs-first-birthday-final"
                }
              >
                OpenSea
              </Link>
            ...
            <div>
              Welcome to the fellowship of the Doge - bound by DOG, governed by
              memes, respected for provenance and protected by us Dogens. bork!
            </div>
          </div>
        );
      ...
      return (
        <div className={css("font-bold", "text-2xl")}>
          ...
          <div>
            You can still claim the original music video{" "}
            <Link href={"/radio"}>here</Link> if you{" "}
            <Link isExternal href={"https://pixels.ownthedoge.com"}>
              Own a Pixel
            </Link>
            .
          </div>
    ...
    return (
      <div className={css("text-gray-400", "font-bold")}>
  ...
  return (
    <PageLayout>
      ...
      <div>
        ...
        <div className={css("mt-4", "text-2xl", "max-w-3xl", "m-auto")}>
          ...
          <div className={css("text-xl", "my-12", "text-center")}>
            DOG turns 1! Memes rule the internet, and 1 year ago we democratized
            the internet by decentralizing the ownership of its most famous
            meme. On August 30th, 2021, PleasrDAO fractionalized The Doge NFT
            giving birth to DOG, a token representing ownership of the OG Doge
            meme.
          </div>
          <div className={css("mt-1", "text-xl", "mb-8", "text-center")}>
            To commemorate 1 year of wholesomeness, fun, dogely values and
            philanthropy on the blockchain, we have launched our very special
            birthday edition soulbound (read more{" "}
            <Link
              isExternal
            ...
            ) token series for supporters who held DOG on-chain (Ethereum,
            Arbitrum, BSC, Polygon, or Optimism) and/or{" "}
            <Link isExternal href="https://pixels.ownthedoge.com">
              Doge Pixels
            </Link>
            , before August 30th, 2022. This NFT is a clip from our music video
            titled “In Doge We Trust” which was created in collaboration with{" "}
            <Link isExternal href="https://twitter.com/Bassjackers">
              Bassjackers
            </Link>
            ,{" "}
            <Link isExternal href="https://twitter.com/pplpleasr1">
              {" "}
              pplpleasr
            </Link>
            , and{" "}
            <Link isExternal href="https://twitter.com/cloudeatr">
              cloudeatr
            </Link>
            . It not only establishes you as a respectable member of our DOG
            universe but also gives you the right to call yourself a Dogen!
          </div>

          <div className={css("text-lg", "text-center")}>
            For the whole month of September, all Doge Pixel holders can claim
            the full music video <Link href="/radio">here</Link>.
          </div>
            ...
            {METDATAS.map((metadata: IMetadata, index) => {
              return (
                <div
                  ...
                  className={css({
                    "opacity-50":
                      (isClaimed && metadata.id !== claimedId) ||
                      (signer && !isInWhiteList),
                  })}
                >
                  <DropShadow
                    onClick={
                      isClaimed || (signer && !isInWhiteList)
                        ? undefined
                        : () => onClickMetadata(metadata)
                    }
                    key={metadata.id}
                    className={css("bg-black[0.05]")}
                  >
                    <video className={css("w-full")} autoPlay={true} loop muted>
                      <source src={metadata.url} key={metadata.id} />
                    </video>
                  </DropShadow>
                  <div className={css("text-center", "mt-4", "text-xl")}>
                    {metadata.name}
                  </div>
                </div>
              );
      ...
      <Modal
        ...
        isOpen={showModal}
        title={
          isClaimed ? "✨   Claimed   ✨" : `✨  ${selectedMetadata.name}  ✨`
        }
        onChange={(val) => setShowModal(val)}
      >
        <div className={css("w-full", "m-auto", "max-w-xl")}>
          <video
            className={css("w-full", "border-2", "border-black")}
            autoPlay={true}
            loop
            muted
          >
        ...
        {justClaimed && (
          <div className={css("text-2xl", "mt-8", "mb-4")}>
        ...
        {!justClaimed && (
          <div className={css("flex", "justify-center", "my-6")}>
            {signer && !isClaimed && (
              <Button
                disabled={!isConnectedToCorrectNetwork}
                isLoading={isClaiming}
                onClick={() => claim()}
              >
                {isConnectedToCorrectNetwork
                  ? "Claim"
                  : `Please change network to: ${targetChain.name}`}
              </Button>
            )}
            {!signer && (
              <div className={css("text-xl")}>Connect wallet to mint</div>
            )}
          </div>
        )}
      </Modal>
