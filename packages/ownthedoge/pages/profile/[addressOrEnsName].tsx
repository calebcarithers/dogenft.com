      ...
      <div className={css("mt-4", "text-3xl", "max-w-3xl", "m-auto", "flex", "justify-center")}>
        <div className={css("flex", "flex-col", "items-center")}>
          ...
          <div
            ...
            <div
              className={css("text-2xl", "font-bold", "text-pixels-yellow-500")}>{store.dogBalance ? formatWithThousandsSeparators(store.dogBalance) : "---"}</div>
        ...
        <Pane title={<Title title={"Doggos"} count={store.doggos.length}/>}>
          <MaxHeightThing>
            {store.hasDoggos && <div className={css("w-full")}>
              {store.doggos.map(doggo => <div key={`doggo-${doggo.metadata.name}`}
                                              className={css("break-words", "overflow-auto", "flex", "justify-center")}>
...
const MaxHeightThing: React.FC<PropsWithChildren<{}>> = ({children}) => {
  return <div className={css("overflow-y-scroll", "flex", "flex-col", "flex-grow")}>
    <div className={css("flex-grow", "flex", "justify-center", "overflow-x-hidden", "py-6")}
...
const NoneFound: React.FC<PropsWithChildren<{}>> = ({children}) => {
  return <div
    className={css("my-8", "text-center", "text-xl", "font-bold", "text-pixels-yellow-400", "flex", "items-center")}>
    {children}
  </div>
}
