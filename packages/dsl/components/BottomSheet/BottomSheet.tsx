...
const BottomSheet = React.forwardRef<BottomSheetRef, BottomSheetProps>(({
    children, 
    snapPoints = ({ minHeight, maxHeight }) => [minHeight, maxHeight/1.2, maxHeight],
    defaultSnap = ({ maxHeight }) => (maxHeight/1.2),
    ...rest}, ref) => {
    return <SpringBottomSheet snapPoints={snapPoints} defaultSnap={defaultSnap} ref={ref} {...rest}>
        <div className={css("p-8", "h-full")}>
            {children}
        </div>
    </SpringBottomSheet>
})
