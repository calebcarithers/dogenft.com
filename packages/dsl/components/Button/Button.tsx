...
const Button: React.FC<PropsWithChildren<ButtonProps>> = ({
  ...
  return <div className={css("relative", "inline-block", "z-10", "h-fit", "select-none", {"w-full": block})} ref={ref}>
    <button disabled={isDisabled} onClick={() => {
      ...
      {children}
      {isLoading && <div
        className={css("absolute", "w-full", "left-0", "top-0", "h-full", "flex", "items-center", "justify-center", "opacity-75", {
          "bg-pixels-yellow-300": type === ButtonType.Primary,
          "bg-gray-900": type === ButtonType.White
        })}>
        <ClipLoader size={25} speedMultiplier={0.5}
                    color={type === ButtonType.Primary ? tailwindconfig?.theme?.extend?.colors?.pixels?.yellow[50] : tailwindconfig?.theme?.extend?.colors?.meme?.yellow}/>
      </div>}
    </button>
            ...
            {(() => {
              ...
              return (
                <div style={{display: 'flex', gap: 12}}>
                  <Dropdown type={isFraction ? DropdownType.White : DropdownType.Primary} open={isDropDownOpen}
