    ...
    IERC1155ReceiverUpgradeable
{
    ...
    function deposit(
        ...
        require(
            _amount > 0,
            "FractionalManager: Amount must be greater than 0"
        );
        require(
            IERC1155(_erc1155Address).balanceOf(msg.sender, _fractionId) >=
                _amount,
            "Fractional: Insufficient balance"
        );
    ...
    function claim(
        ...
        require(
            IERC1155(_erc1155Address).balanceOf(address(this), _fractionId) >=
                _pixelIds.length,
            "Insufficient balance"
        );
        for (uint256 i = 0; i < _pixelIds.length; i++) {
            require(
                !pixelClaimed[_erc1155Address][_fractionId][_pixelIds[i]],
                "Pixel already claimed"
            );
            require(
                IERC721(pixelAddress).ownerOf(_pixelIds[i]) == msg.sender,
                "Not pixel owner"
            );
    ...
    function withdraw(address _erc1155Address, uint256 _fractionId)
        public
        onlyOwner
    {
        uint256 balance = IERC1155(_erc1155Address).balanceOf(
            address(this),
            _fractionId
        );
        IERC1155(_erc1155Address).safeTransferFrom(
    ...
    function getIsClaimOpen(address _erc1155Address, uint256 _tokenId)
        public
        view
        returns (bool)
    {
