...
const DonateDialog = () => {
  const state = useAppStore((state) => state);
  return (
    <div className={css("relative", "pb-6", "h-full")}>
      {state.donationModalView !== DonationModalView.Donate && (
        <div
          className={css(
            "cursor-pointer",
            "text-4xl",
      ...
      <div className={css("mt-16")}>
        ...
        <>
          ...
          {state.donationModalView === DonationModalView.Warning && (
            <WarningView />
          )}
          {state.donationModalView === DonationModalView.Address && (
            <AddressView />
          )}
        </>
        {/* </CSSTransition> */}
      ...
      <div className={css("w-full")}>
        ...
        >
          <div className={css("flex", "items-center", "justify-center")}>
            ...
            <div className={css("text-3xl", "font-normal", "p-3")}>
              ETH / ERC20s
            </div>
          </div>
        </Button>
      ...
      <div className={css("flex", "justify-center")}>
        ...
        >
          <Image
            ...
            width={100}
            height={100}
            src={"/images/doge-actually.jpeg"}
            alt={"doge actually"}
            priority
          />
        </div>
      </div>
      <div className={css("text-2xl", "text-center")}>{description}</div>
      <Button
        block
        onClick={() => state.setDonationModalView(DonationModalView.Address)}
      >
  ...
  const depositDetails = useMemo(() => {
    if (state.donationModalCurrency === DonationCurrency.Ethereum) {
      return {
        ...
        onCopy: () =>
          gaEvent({ action: GaActions.EthereumAddressCopy, params: {} }),
      };
    } else if (state.donationModalCurrency === DonationCurrency.Dogecoin) {
      return {
        ...
        onCopy: () =>
          gaEvent({ action: GaActions.DogecoinAddressCopy, params: {} }),
      };
    }
  }, [
    state.donationModalCurrency,
    state.ethereumDonationAddress,
    state.dogeDonationAddress,
  ]);
                  ...
                  navigator.clipboard
                    ...
                    .then(() => {
                      toast(`✅ ${depositDetails?.title} copied`, {
                        toastId: `donate-${depositDetails?.address}`,
                      });
                    });
