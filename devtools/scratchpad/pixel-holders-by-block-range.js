...
const PXContract = new ethers.Contract(
  mainnetContractInfo["address"],
  mainnetContractInfo["abi"],
  provider
);
...
async function getAddressToOwnershipMap(fromBlock, toBlock) {
  ...
  const logs = getEthLogs(
    fromBlock,
    toBlock,
    PXContract.filters.Transfer(null, null)
  );

  for (const tx of logs) {
    const { from, to } = tx.args;
    const tokenID = tx.args.tokenId.toNumber();
    addressToPuppers = await addRemoveAddresses(
      addressToPuppers,
      from,
      to,
      tokenID
...
const main = async () => {
  ...
  console.log((await dater.getDate("2022-03-31T23:59:59Z")).block);
